/**
 * @file ResourceHolder_test.cxx  Unit Tests for Resource disabling logic
 *
...
#define BOOST_TEST_MODULE ResourceHolder_test // NOLINT
...
BOOST_AUTO_TEST_SUITE(ResourceHolder_test)
...
BOOST_AUTO_TEST_CASE(simple_resource_set){
  ...
  std::vector<const DummyApplication*> dummy_resources;
  ...
  segment_conf_obj.set_objs("applications", resource_config_objects);

  auto root = confdb.get<Segment>(segment_conf_obj);

  std::vector<const conffwk::ConfigObject*> disabled_config_objects
    {resource_config_objects[1]};

  conffwk::ConfigObject rh_conf_obj;
  confdb.create(oksfile, "ResourceHolder", "rh", rh_conf_obj);
  rh_conf_obj.set_objs("disabled", disabled_config_objects);
  rh_conf_obj.set_obj("segment", &segment_conf_obj);
  auto rh = confdb.get<ResourceHolder>(rh_conf_obj);

  BOOST_CHECK( rh->is_enabled(root) );
  BOOST_CHECK( rh->is_enabled(dummy_resources[0]) );
  BOOST_CHECK( !rh->is_enabled(dummy_resources[1]) );

  disabled_config_objects = resource_config_objects;
  confdb.create(oksfile, "ResourceHolder", "rh-1", rh_conf_obj);
  rh_conf_obj.set_objs("disabled", disabled_config_objects);
  rh_conf_obj.set_obj("segment", &segment_conf_obj);
  auto rh1 = confdb.get<ResourceHolder>(rh_conf_obj);
  BOOST_CHECK( !rh1->is_enabled(root) );
  BOOST_CHECK( !rh1->is_enabled(dummy_resources[1]) );
}
#if 0
...
BOOST_AUTO_TEST_CASE(detector_to_daq){
  ...
  conf_obj.set_objs("dummy_sender", sender_conf_objs);
  ...
  // All streams of both sender disabled
